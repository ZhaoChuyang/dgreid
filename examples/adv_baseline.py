...
def create_model(args):
    model = models.create(args.arch, num_features=args.features, norm=False, dropout=args.dropout, 
                          num_classes=args.nclass, num_domains=args.num_domains)
...
def main_worker(args):
    ...
    args.num_domains = dataset_source.num_domains
    ...
    comm_params = []
    adv_params = []
    for module in model.module.comm_modules:
        comm_params += [{"params": value} for _, value in module.named_parameters() if value.requires_grad]
        adv_params += [{"params": value} for _, value in module.named_parameters() if value.requires_grad]
    for module in model.module.adv_modules:
        adv_params += [{"params": value} for _, value in module.named_parameters() if value.requires_grad]

    comm_optimizer = torch.optim.Adam(comm_params, lr=args.lr, weight_decay=args.weight_decay)
    adv_optimizer = torch.optim.Adam(adv_params, lr=args.lr, weight_decay=args.weight_decay)
    comm_lr_scheduler = torch.optim.lr_scheduler.StepLR(comm_optimizer, step_size=args.step_size, gamma=0.1)
    adv_lr_scheduler = torch.optim.lr_scheduler.StepLR(adv_optimizer, step_size=args.step_size, gamma=0.1)

    optimizers = {'comm': comm_optimizer, 'adv': adv_optimizer}
    lr_schedulers = {'comm': comm_lr_scheduler, 'adv': adv_lr_scheduler}

    # Trainer
    trainer = AdvTrainer(model, args.nclass, adv_update_steps=args.adv_update_steps, margin=args.margin)

    epoch_phases = ['adv'] * 5 + ['mix'] * 55 + ['normal'] * 40

    for epoch in range(args.epochs):
        phase = epoch_phases[epoch]
        train_loader_source.new_epoch()
        # train_loader_target.new_epoch()
        trainer.train(epoch, train_loader_source, optimizers, print_freq=args.print_freq, train_iters=args.iters, phase=phase)
...
if __name__ == '__main__':
    ...
    # adversarial discriminator update periods
    parser.add_argument('--adv_update_steps', type=int, default=2)
