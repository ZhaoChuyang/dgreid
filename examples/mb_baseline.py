...
from reid.trainers import Baseline_Trainer, IDM_Trainer, Base_Trainer, Memory_Trainer
...
def main_worker(args):
    ...
    memory = MemoryClassifier(model.module.num_features, source_centers.shape[0],
                               temp=args.temp, momentum=args.momentum).cuda()
    memory.features = source_centers
    memory.labels = torch.arange(dataset_source.num_train_pids).cuda()
    memory = nn.DataParallel(memory)
    ...
    # Trainer
    trainer = Memory_Trainer(model, args.nclass, memory, margin=args.margin)
